...
/**
 * Abstract Compiler plugin for validating Stomp Listener services.
 *
 * @since 0.995.0
 */
public abstract class StompServiceCompilerPlugin extends AbstractTransportCompilerPlugin {
    ...
    public void process(ServiceNode serviceNode, List<AnnotationAttachmentNode> annotations) {
        ...
        if (!annotations.isEmpty()) {
            dlog.logDiagnostic(Diagnostic.Kind.ERROR, serviceNode.getPosition(),
                    "The Stomp service does not have a service annotation");
        }
    ...
    private void validate(BLangFunction resource) {
        ...
        if (!StompConstants.MESSAGE_OBJ_FULL_NAME.equals(paramDetails.get(1).type.toString())) {
            dlog.logDiagnostic(Diagnostic.Kind.ERROR, resource.pos,
                    INVALID_RESOURCE_SIGNATURE_FOR + resource.getName().getValue() +
                            ...
                            StompConstants.MESSAGE_OBJ_FULL_NAME);
        }
    }
